...
##PROFILE DESIGNS
profileDesign(
  phi=seq(from=0.003,to=0.2,length=10),
  lower=c(beta1=11.2, beta2=0.2, beta3=0.4,beta11=0.145,od=0.04,sigma=0.01,sir_fixed_params ),
  upper=c(beta1=12.2, beta2=0.3, beta3=0.5,beta11=0.165,od=0.15,sigma=0.1,sir_fixed_params ),
  nprof=2
) -> pd_phi
...
stew(file="gamma_profile_phi_data_diff-%d.rda",{
  ...
  t_global <- system.time({
    mifs_global <- foreach(p=iter(pd,"row"),.packages='pomp', .combine=c, .options.multicore=mcopts) %dopar% {
      mif2(
        ...
        transform=TRUE,
        rw.sd=rw.sd(beta1=0.002,beta2=0.002,beta3=0.002,beta11=0.001,od=0.01,sigma=0.01)
      )
    }
  })
...
stew(file="gamma_profile_phi_lik_data_diff-%d.rda",{
  
  t_global_eval <- system.time({
    liks_global <- foreach(i=1:20,.packages='pomp',.combine=rbind, .options.multicore=mcopts) %dopar% {
...
lambda_spacetime <- 1
spacetime_mcap_beta <- mcap(lp=results$logLik,parameter=results$phi,lambda=lambda_spacetime)
