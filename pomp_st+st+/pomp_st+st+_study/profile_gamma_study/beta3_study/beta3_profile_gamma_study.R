...
##PROFILE DESIGNS
profileDesign(
  beta3=seq(from=0.35,to=0.5,length=10),
  lower=c(beta1=12.2, beta2=0.2, beta11=0.145,phi=0.05,od=0.44,sigma=0.01,sir_fixed_params ),
  upper=c(beta1=13.2, beta2=0.3, beta11=0.155,phi=0.15,od=0.55,sigma=0.1,sir_fixed_params ),
  nprof=2
) -> pd_beta3
...
stew(file="gamma_profile_beta3_study-%d.rda",{
  ...
  t_global <- system.time({
    mifs_global <- foreach(p=iter(pd,"row"),.packages='pomp', .combine=c, .options.multicore=mcopts) %dopar% {
      mif2(
        ...
        transform=TRUE,
        rw.sd=rw.sd(beta1=0.002,beta2=0.002,beta11=0.001,phi=0.01,od=0.01,sigma=0.01)
      )
    }
  })
...
stew(file="gamma_profile_beta3_lik_study-%d.rda",{
...
lambda_spacetime <- 1 
spacetime_mcap_beta <- mcap(lp=results$logLik,parameter=results$beta3,lambda=lambda_spacetime)
