...
##PROFILE DESIGNS
profileDesign(
  beta11=seq(from=0.135,to=0.165,length=10),
  lower=c(beta1=12.2, beta2=0.2, beta3=0.4,phi=0.05,od=0.44,sigma=0.01,sir_fixed_params ),
  upper=c(beta1=13.2, beta2=0.3, beta3=0.5,phi=0.15,od=0.55,sigma=0.1,sir_fixed_params ),
  nprof=2
) -> pd_beta11
...
stew(file="gamma_profile_beta11_study-%d.rda",{
  ...
  t_global <- system.time({
    mifs_global <- foreach(p=iter(pd,"row"),.packages='pomp', .combine=c, .options.multicore=mcopts) %dopar% {
      mif2(
        ...
        transform=TRUE,
        rw.sd=rw.sd(beta1=0.002,beta2=0.002,beta3=0.002,phi=0.01,od=0.01, sigma=0.01)
      )
    }
  })
...
stew(file="gamma_profile_beta11_lik_study-%d.rda",{
...
lambda_spacetime <- 1 
spacetime_mcap_beta <- mcap(lp=results$logLik,parameter=results$beta11,lambda=lambda_spacetime)
