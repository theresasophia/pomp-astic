...
##PROFILE DESIGNS
profileDesign(
  sigma=seq(from=0.01,to=0.1,length=10),
  lower=c(beta1=12.2, beta2=0.2, beta3=0.4,beta11=0.145,od=0.44,phi=0.01,sir_fixed_params ),
  upper=c(beta1=13.2, beta2=0.3, beta3=0.5,beta11=0.165,od=0.55,phi=0.15,sir_fixed_params ),
  nprof=2
) -> pd_phi
...
stew(file="gamma_profile_sigma_study-%d.rda",{
  ...
  t_global <- system.time({
    mifs_global <- foreach(p=iter(pd,"row"),.packages='pomp', .combine=c, .options.multicore=mcopts) %dopar% {
      mif2(
        ...
        transform=TRUE,
        rw.sd=rw.sd(beta1=0.002,beta2=0.002,beta3=0.002,beta11=0.001,od=0.01,phi=0.01)
      )
    }
  })
...
stew(file="gamma_profile_sigma_lik_study-%d.rda",{
...
lambda_spacetime <- 1 
spacetime_mcap_beta <- mcap(lp=results$logLik,parameter=results$sigma,lambda=lambda_spacetime)
