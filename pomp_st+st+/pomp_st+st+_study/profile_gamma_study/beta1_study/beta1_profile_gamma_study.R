...
##PROFILE DESIGNS
profileDesign(
  beta1=seq(from=11.5,to=13.2,length=10),
  lower=c(beta2=0.2, beta3=0.4, beta11=0.145,phi=0.05,od=0.44,sigma=0.001,sir_fixed_params ),
  upper=c(beta2=0.3, beta3=0.5, beta11=0.155,phi=0.15,od=0.55,sigma=0.1,sir_fixed_params ),
  nprof=2
) -> pd_beta1
...
stew(file="gamma_profile_beta1_studylo-%d.rda",{
  ...
  t_global <- system.time({
    mifs_global <- foreach(p=iter(pd,"row"),.packages='pomp', .combine=c, .options.multicore=mcopts) %dopar% {
      mif2(
        ...
        transform=TRUE,
        rw.sd=rw.sd(beta2=0.002,beta3=0.002,beta11=0.001,phi=0.01,od=0.01,sigma=0.01)
      )
    }
  })
...
stew(file="gamma_profile_beta1_lik_studylo-%d.rda",{
