## simulation study to calucate the MLE for model stst+ with iterated filtering using the pomp package
## details can be found in the manuscript "pomp-astic inference methods for epidemic models 
## illustarted on German rotavirus surveillance data" by T.Stocks, T. Britton, M. Höhle
...
#observational model, observations are negative binomial distributed
...
#first observation in the data frame
...
params <- c(beta_ana(help_param), beta11=0.15, phi=0.1, params_fixed, first_data, od=0.5)

# to and from estimation scale (phi should be in [0,2*pi])
toEst<- Csnippet("
                 Tbeta1  = log(beta1);
                 Tbeta2  = log(beta2);
                 Tbeta3  = log(beta3);
...
# define the pomp object and initalize 6 years prior to first observation so system can equilibrate
pomp(data = dat,
     times="time",
     t0=1-6*52,
     dmeasure = dmeas,
...
# how to create study data:
# change data to original data frame, then:
...
# write.table(rota_study, file = ".../rota_study.txt" )
...
#hypercube to draw starting values from
sir_box <- rbind(
  beta1=c(12,16),
  beta2=c(0.2,0.4),
  beta3=c(0.3,0.5),
...
#global mif2 search
stew(file="nbin_pomp_box_studylo-%d.rda",{
  ...
  t_global <- system.time({
    mifs_global <- foreach(i=1:20,.packages='pomp', .combine=c, .options.multicore=mcopts) %dopar% {
      mif2(
        ...
        transform=TRUE,
        rw.sd=rw.sd(beta1=0.002,beta2=0.002,beta3=0.002,beta11=0.001,phi=0.01,od=0.01)
      )
    }
  })
},seed=1270401374,kind="L'Ecuyer")

#diagnostic plot of the mif2 output
mifs_global %>%
...
# calulcation of loglikelihood of each mif2 output 
stew(file="nbin_pomp_lik_studylo-%d.rda",{
...
#logliklihoods of mif2 output
liks_global

#choose parameter constellation with highest loglikelihood
best <- which.max(liks_global[,1])
#maximum loglikelihood of mle
