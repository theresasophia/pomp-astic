  ...
  t_global <- system.time({
    mf <- foreach(p=iter(pd,"row"),
                  .packages='pomp', .combine=c, .options.multicore=mcopts) %dopar% {
                    traj.match(study,
                               start=unlist(p),
                               est=c("beta1","beta2","beta3","beta11"),
...
pairs(~logLik+phi,data=subset(results_global,logLik>max(logLik)-1.92))
...
results_global <- res(liks, coef)
pairs(~logLik+phi,data=results_global)

results_global%>% 
  mutate(phi=signif(phi,8)) %>%
  ddply(~phi,subset,logLik==max(logLik)) %>%
  ggplot(aes(x=phi,y=logLik))+geom_point() +
  geom_quantile(formula=y~1+x+I(x^2), quantiles=0.9,col="blue")

# linear interpolation
interp <- with(results_global,approxfun(phi,logLik))
uni_fun<- function(x){ interp(x) - (max(results_global$logLik)-1.92)}
x<- seq(0.0,0.5,0.001)
plot(x,uni_fun(x),type="l")
library("rootSolve")
int_phi <- sprintf("[%.3f, %.3f]", uniroot.all(uni_fun, lower=0., upper=0.5)[1],uniroot.all(uni_fun, lower=0., upper=0.2)[2])
save(int_phi,file = "~/Dropbox/AAPAPER/R_code/traj.match/det_pois_STUDY/phi_pois.rda")
