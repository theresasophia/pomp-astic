...
###################beta11###################################################
pd <- pd_beta11
stew(file="prof_gam_beta11_study_first-%d.rda",{
...
pairs(~logLik+beta11,data=subset(results_global,logLik>max(logLik)-1.92))

stew(file="prof_gam_beta11_study_second-%d.rda",{
...
results_global <- res(liks, coef)
pairs(~logLik+beta11,data=results_global)

results_global%>% 
  mutate(beta11=signif(beta11,8)) %>%
  ddply(~beta11,subset,logLik==max(logLik)) %>%
  ggplot(aes(x=beta11,y=logLik))+geom_point() +
  geom_quantile(formula=y~1+x+I(x^2), quantiles=0.9,col="blue")

# linear interpolation
interp <- with(results_global,approxfun(beta11,logLik))
uni_fun<- function(x){ interp(x) - (max(results_global$logLik)-1.92)}
x<- seq(0.14,0.15,0.001)
plot(x,uni_fun(x),type="l")
library("rootSolve")
int_beta11 <- sprintf("[%.3f, %.3f]", uniroot.all(uni_fun, lower=0.14, upper=0.16)[1],uniroot.all(uni_fun, lower=0.14, upper=0.16)[2])
save(int_beta11,file = "~/Dropbox/AAPAPER/R_code/traj.match/det_pois_STUDY/beta11_pois.rda")
