...
###################BETA2###################################################
pd <- pd_beta2
stew(file="prof_gam_beta2_study_first-%d.rda",{
...
pairs(~logLik+beta2,data=subset(results_global,logLik>max(logLik)-10))

stew(file="prof_gam_beta2_study_second-%d.rda",{
...
results_global <- res(liks, coef)
pairs(~logLik+beta2,data=results_global)

results_global%>% 
  mutate(beta2=signif(beta2,8)) %>%
  ddply(~beta2,subset,logLik==max(logLik)) %>%
  ggplot(aes(x=beta2,y=logLik))+geom_point() +
  geom_quantile(formula=y~1+x+I(x^2), quantiles=0.9,col="blue")

# linear interpolation
interp <- with(results_global,approxfun(beta2,logLik))
uni_fun<- function(x){ interp(x) - (max(results_global$logLik)-1.92)}
x<- seq(0.1,0.5,0.001)
plot(x,uni_fun(x),type="l")
library("rootSolve")
int_beta2 <- sprintf("[%.3f, %.3f]", uniroot.all(uni_fun, lower=0.2, upper=0.3)[1],uniroot.all(uni_fun, lower=0.2, upper=0.3)[2])
save(int_beta2,file = "~/Dropbox/AAPAPER/R_code/traj.match/det_pois_STUDY/beta2_pois.rda")
