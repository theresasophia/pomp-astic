...
load(file="~/Dropbox/AAPAPER/R_code/EXTENSION_traj.match/coef_traj_pois 100 .rda")
...
coef_fr%>%
  subset(select=c("beta1" , "beta2" , "beta3",  "beta11", "phi" )) %>%
  melt(id=c("sample"))  %>%
  mutate(Var2= factor(Var2))-> coef_fr

new_data <- merge(coef_fr,mle_true,  by.x = "Var2", by.y = "variable")

cairo_ps(file="~/Dropbox/AAPAPER/Revision_Biostatistics/ShareLaTeX/Revision_biostatistics/boxplot_pois.eps", width=17, height=10)
new_data%>%
