...
###################BETA3###################################################
pd <- pd_beta3
stew(file="prof_gam_beta3_study_first-%d.rda",{
  ...
  t_global <- system.time({
    mf <- foreach(p=iter(pd,"row"),
                  .packages='pomp', .combine=c, .options.multicore=mcopts) %dopar% {
                    traj.match(study,
                               start=unlist(p),
                               est=c("beta1","beta2","beta11","phi","od"),
...
pairs(~logLik+beta3,data=subset(results_global,logLik>max(logLik)-100))

     
stew(file="prof_gam_beta3_study_second-%d.rda",{
...
results_global <- res(liks, coef)
with(results_global,plot(beta3,logLik))
#with(results_global_2,points(beta3,logLik,pch=5,col="green"))
pairs(~logLik+beta3,data=results_global)

results_global%>% 
  mutate(beta3=signif(beta3,8)) %>%
  ddply(~beta3,subset,logLik==max(logLik)) %>%
  ggplot(aes(x=beta3,y=logLik))+geom_point() +
  geom_quantile(formula=y~1+x+I(x^2), quantiles=0.9,col="blue")

# linear interpolation
interp <- with(results_global,approxfun(beta3,logLik))
uni_fun<- function(x){ interp(x) - (max(results_global$logLik)-1.92)}
x<- seq(0.,0.5,0.001)
plot(x,uni_fun(x),type="l")
library("rootSolve")
int_beta3 <- sprintf("[%.3f, %.3f]", uniroot.all(uni_fun, lower=0, upper=2)[1],uniroot.all(uni_fun, lower=0, upper=1)[2])
save(int_beta3,file = "~/Dropbox/AAPAPER/R_code/traj.match/det_nbin_STUDY/beta3_nbin.rda")
