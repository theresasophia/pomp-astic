  ...
  t_global <- system.time({
    mf <- foreach(p=iter(pd,"row"),
                  .packages='pomp', .combine=c, .options.multicore=mcopts) %dopar% {
                    traj.match(study,
                               start=unlist(p),
                               est=c("beta1","beta3","beta11","phi","od"),
...
library("rootSolve")
int_beta2 <- sprintf("[%.3f, %.3f]", uniroot.all(uni_fun, lower=0, upper=2)[1],uniroot.all(uni_fun, lower=0, upper=1)[2])
save(int_beta2,file = "~/Dropbox/AAPAPER/R_code/traj.match/det_nbin_STUDY/beta2_nbin.rda")
