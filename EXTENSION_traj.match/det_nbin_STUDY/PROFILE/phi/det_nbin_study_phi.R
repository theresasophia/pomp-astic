  ...
  t_global <- system.time({
    mf <- foreach(p=iter(pd,"row"),
                  .packages='pomp', .combine=c, .options.multicore=mcopts) %dopar% {
                    traj.match(study,
                               start=unlist(p),
                               est=c("beta1","beta2","beta3","beta11","od"),
...
library("rootSolve")
int_phi <- sprintf("[%.3f, %.3f]", uniroot.all(uni_fun, lower=0, upper=2)[1],uniroot.all(uni_fun, lower=0, upper=1)[2])
save(int_phi,file = "~/Dropbox/AAPAPER/R_code/traj.match/det_nbin_STUDY/phi_nbin.rda")
