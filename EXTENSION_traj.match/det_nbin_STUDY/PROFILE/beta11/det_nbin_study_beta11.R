  ...
  t_global <- system.time({
    mf <- foreach(p=iter(pd,"row"),
                  .packages='pomp', .combine=c, .options.multicore=mcopts) %dopar% {
                    traj.match(study,
                               start=unlist(p),
                               est=c("beta1","beta2","beta3","phi","od"),
...
pairs(~logLik+beta11,data=subset(results_global,logLik>max(logLik)-5))
...
library("rootSolve")
int_beta11 <- sprintf("[%.3f, %.3f]", uniroot.all(uni_fun, lower=0, upper=2)[1],uniroot.all(uni_fun, lower=0, upper=1)[2])
save(int_beta11,file = "~/Dropbox/AAPAPER/R_code/traj.match/det_nbin_STUDY/beta11_nbin.rda")
