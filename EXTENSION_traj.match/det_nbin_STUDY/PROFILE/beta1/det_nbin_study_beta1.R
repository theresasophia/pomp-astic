  ...
  t_global <- system.time({
    mf <- foreach(p=iter(pd,"row"),
                  .packages='pomp', .combine=c, .options.multicore=mcopts) %dopar% {
                    traj.match(study,
                               start=unlist(p),
                               est=c("beta2","beta3","beta11","phi","od"),
...
pairs(~logLik+beta1,data=subset(results_global,logLik>max(logLik)-100))
...
library("rootSolve")
int_beta1<- sprintf("[%.3f, %.3f]", uniroot.all(uni_fun, lower=0, upper=20)[1],uniroot.all(uni_fun, lower=0, upper=20)[2])
save(int_beta1,file = "~/Dropbox/AAPAPER/R_code/traj.match/det_nbin_STUDY/beta1_nbin.rda")
