...
profileDesign(
  ...
  lower=c(beta1=12.2, beta2=0.2, beta3=0.4,phi=0.05,sigma=0.01,sir_fixed_params ),
  upper=c(beta1=13.2, beta2=0.3, beta3=0.5,phi=0.15,sigma=0.1,sir_fixed_params ),
  nprof=2
) -> pd_beta11
...
stew(file="st+st_profile_beta11_study_diff-%d.rda",{
  ...
  t_global <- system.time({
    mifs_global <- foreach(p=iter(pd,"row"),.packages='pomp', .combine=c, .options.multicore=mcopts) %dopar% {
      mif2(
        ...
        transform=TRUE,
        rw.sd=rw.sd(beta1=0.002,beta2=0.002,beta3=0.002,phi=0.01, sigma=0.01)
      )
    }
  })
...
stew(file="st+st_profile_beta11_lik_study_diff-%d.rda",{
