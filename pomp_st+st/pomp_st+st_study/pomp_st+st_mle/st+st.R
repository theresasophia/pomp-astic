...
# write.table(rota_study, file = "~/Dropbox/AAPAPER/R_code/pomp_pois/pois-gamma_study/mle_pois_gamma_study/rota_study.txt" )
...
stew(file=paste("st+st_mle",sep = "",i,"-%d_5000.rda"),{
  ...
  t_global <- system.time({
    mifs_global <- foreach(i=1:cores,.packages='pomp', .combine=c, .options.multicore=mcopts) %dopar% {
      mif2(
        ...
        transform=TRUE,
        rw.sd=rw.sd(beta1=0.002,beta2=0.002,beta3=0.002,beta11=0.001,phi=0.01, sigma=0.01)
      )
    }
  })
...
stew(file=paste("st+st_loglik",sep = "",i,"-%d_5000.rda"),{
...
loglik_mle <-round(liks_global[best,],2)

coef_mle <- round(coef(mifs_global[[best]]),3)
n_params <- 6
aic <- 2*(n_params-loglik_mle)

res<- c(loglik=loglik_mle,aic=aic,mle=coef_mle)
