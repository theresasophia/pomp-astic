...
stew(file="mle_gamma_data_pois-%d.rda",{
...
library("grDevices")
#cairo_ps(file="diagnostic_gammapois_data.eps", width=17, height=10)
  ...
  mutate(variable = recode(variable, beta3 = "beta[3]")) %>%
    mutate(variable = recode(variable, sigma = "sigma")) %>%
  mutate(variable = recode(variable, phi = "phi"))-> df
...
# df2$age <- factor(rep(c("children", "adult", "elderly"), each = dim(quantile_obs)[2] - 1),
...
# 
# #pdf(file="~/Dropbox/AAPAPER/Latex/mle_pois_gamma_data.pdf", width=17, height=9)
# ggplot(df2) +
